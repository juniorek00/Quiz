        ...
        <div className="bg-surface p-8 rounded-2xl border border-white/10 shadow-2xl text-center relative flex flex-col items-center justify-center min-h-[300px]">
          ...
          {phase !== 'FINISHED' ? (
            <div className="w-full animate-fade-in border-t border-white/10 pt-6">
              <p className="text-gray-400 mb-4 uppercase tracking-widest text-sm">Wybierz drużyny (1–{entities.length}):</p>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {entities.map(entity => (
                  <button key={entity.id} onClick={() => handleTeamClick(entity.id, entity.name)} aria-pressed={Boolean(selectedEntities.find(s => s.id === entity.id))} className={`py-3 px-4 ${selectedEntities.find(s => s.id === entity.id) ? 'bg-green-600/30 border-green-500' : 'bg-white/5 hover:bg-purple-600 hover:text-white'} border border-white/10 rounded-xl font-bold transition-all truncate flex items-center gap-3`}>
                    {mode === 'TEAM' ? (
                      entity.image ? (
                        <img
                          src={entity.image}
              ...
              {selectedEntities.length > 0 ? (
                <div className="mt-4 flex flex-col items-center gap-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setSelectedEntities([])} className="bg-transparent border border-white/10 p-3 rounded-xl text-gray-400 hover:bg-white/5">Anuluj</button>
                  </div>
                </div>
              ) : (
                <div className="mt-4">
