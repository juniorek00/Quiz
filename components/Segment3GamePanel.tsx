...
import { Trophy, ArrowRight, XCircle, Music, Edit2, PlayCircle, PauseCircle, Volume2, Save, X, Star, CheckCircle2, Users, LifeBuoy } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 p-4 max-w-6xl mx-auto">
      ...
      <div className="lg:col-span-3 space-y-6">
        ...
        <div className="bg-surface p-8 rounded-2xl border border-white/10 shadow-2xl text-center relative flex flex-col items-center justify-center min-h-[300px]">
          <div className="mb-8">
            <div className={`w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-4 border-4 transition-all ${phase === 'PLAYING_QUESTION' ? 'border-purple-500 bg-purple-500/20 animate-pulse' : phase === 'PLAYING_ANSWER' ? 'border-green-500 bg-green-500/20' : 'border-gray-600 bg-gray-800'}`}>
                 {phase === 'PLAYING_QUESTION' || phase === 'PLAYING_ANSWER' ? <Volume2 className="w-16 h-16 text-white" /> : <PlayCircle className="w-16 h-16 text-gray-500" />}
            </div>
            <h3 className="text-2xl font-bold mb-2">{phase === 'PLAYING_QUESTION' ? "Odtwarzanie pytania..." : phase === 'PLAYING_ANSWER' ? "Odtwarzanie odpowiedzi..." : phase === 'FINISHED' ? "Runda Zakończona" : "Oczekiwanie..."}</h3>
            {phase === 'PLAYING_QUESTION' && <p className="text-purple-400 animate-pulse">Słuchaj uważnie...</p>}
            {phase === 'WAITING_FOR_REVEAL' && <p className="text-gray-400">Koniec fragmentu. Kto zgłosił się pierwszy?</p>}
          </div>

          {phase !== 'FINISHED' && (
              <button onClick={playAnswer} disabled={!showAnswerButtonEnabled || phase === 'PLAYING_ANSWER'} className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition-all mb-8 ${showAnswerButtonEnabled && phase !== 'PLAYING_ANSWER' ? 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-500/25' : 'bg-gray-700 text-gray-500 cursor-not-allowed'}`}>
          ...
          {phase !== 'FINISHED' ? (
            ...
            <div className="w-full bg-green-500/10 border border-green-500/30 p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in">
               <div className="text-left">
                  ...
                  {activePair.title && <div className="text-sm text-gray-500 mt-1">Utwór: {activePair.title}</div>}
               </div>
