  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (correctLetter === 'A') correctIndex = 0;
        ...
        // Jeśli zamiast litery podano tekst odpowiedzi, spróbuj znaleźć dopasowanie
        const normalized = (s: string) => s.trim().toLowerCase();
        correctIndex = answers.findIndex(a => normalized(a) === normalized(rawCorrect));
        if (correctIndex === -1) {
          // Spróbuj dopasować, gdy w CSV jest częściowy tekst
          correctIndex = answers.findIndex(a => normalized(a).includes(normalized(rawCorrect)) || normalized(rawCorrect).includes(normalized(a)));
        }
      }

      if (correctIndex === -1) {
        console.warn(`Nie można ustalić poprawnej odpowiedzi dla pytania ID ${id} (wczytano: "${rawCorrect}"). Domyślnie ustawiam odpowiedź A.`);
        correctIndex = 0;
      }
